...
{
    private static final String TAG = "ID3v24Tag";
               ...
               if(o instanceof AbstractID3v2Frame)
               {
                   ...
                   if (newFrame.getBody() instanceof FrameBodyTDRC)
                   {
                       ...
                       else
                       {
                           ...
                           if (DEBUG)
                               Log.e(TAG, "Found duplicate TDRC frame in invalid situation,discarding:" + newFrame.getIdentifier());
                       }
                   }
                   else
                   {
        ...
        if (mp3tag != null)
        {
            ...
            else if (mp3tag instanceof AbstractLyrics3)
            {
                ...
                while (iterator.hasNext())
                {
                    ...
                    {
                        if (DEBUG)
                            Log.e(TAG, "Unable to convert Lyrics3 to v24 Frame:Frame Identifier");
                    }
                }
            }
        }
    ...
    private void readHeaderFlags(ByteBuffer byteBuffer) throws TagException
    {
        ...
        {
            if (DEBUG)
                Log.e(TAG, ErrorMessage.ID3_TAG_FOOTER.getMsg(getLoggingFilename()));
        }
    }
    ...
    public void read(ByteBuffer byteBuffer) throws TagException
    {
        ...
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":" + "Reading ID3v24 tag");
        ...
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":" + "Reading tag from file size set in header is" + size);
    ...
    protected void readFrames(ByteBuffer byteBuffer, int size)
    {
        ...
        while (byteBuffer.position() <= size)
        {
            ...
            {
                if (DEBUG)
                    Log.i(TAG, getLoggingFilename() + ":" + "Invalid Frame Identifier:" + ifie.getMessage());
                this.invalidFrames++;
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Writing tag to channel");
