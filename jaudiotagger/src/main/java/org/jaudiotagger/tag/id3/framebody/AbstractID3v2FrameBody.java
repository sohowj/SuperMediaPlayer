...
{
    private static final String TAG = "AbstractID3v2FrameBody";
    ...
    public void read(ByteBuffer byteBuffer) throws InvalidTagException
    {
        int size = getSize();
        if (DEBUG)
            Log.i(TAG, "Reading body for" + this.getIdentifier() + ":" + size);
        ...
        {
            if (DEBUG)
                Log.i(TAG, "offset " + offset + " for " + object.getClass().getName());
            ...
            {
                if (DEBUG)
                    Log.e(TAG, "Invalid Size for FrameBody");
            ...
            {
                if (DEBUG)
                    Log.e(TAG, "Problem reading datatype within Frame Body:" + e.getMessage());
                throw e;
            }
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Writing frame body for" + this.getIdentifier() + ":Est Size:" + size);
        ...
        setSize();
        if (DEBUG)
            Log.i(TAG, "Written frame body for" + this.getIdentifier() + ":Real Size:" + size);
