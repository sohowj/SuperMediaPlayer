...
{
    private static final String TAG = "AbstractID3v2Frame";
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Creating empty frame of type" + identifier);
        ...
        {
            if (DEBUG)
                Log.e(TAG, "InstantiationException:" + identifier, ie);
        ...
        {
            if (DEBUG)
                Log.e(TAG, "IllegalAccessException:" + identifier, iae);
        ...
        if (DEBUG)
            Log.i(TAG, "Created empty frame of type" + identifier);
    }
            ...
            throws InvalidFrameException, InvalidDataTypeException
    {
        ...
        if (DEBUG)
            Log.i(TAG, "Creating framebody:start");

        AbstractID3v2FrameBody frameBody;
        try
        {
            Class<AbstractID3v2FrameBody> c = (Class<AbstractID3v2FrameBody>) Class.forName("org.jaudiotagger.tag.id3.framebody.FrameBody" + identifier);
            Class<?>[] constructorParameterTypes = {ByteBuffer.class, Integer.TYPE};
        ...
        {
            if (DEBUG)
                Log.i(TAG, getLoggingFilename() + ":" + "Identifier not recognised:" + identifier + " using FrameBodyUnsupported");
            try
            {
        ...
        {
            if (DEBUG)
                Log.e(TAG, getLoggingFilename() + ":" + "An error occurred within abstractID3v2FrameBody for identifier:" + identifier + ":" + ite.getCause().getMessage());
        ...
        {
            if (DEBUG)
                Log.e(TAG, getLoggingFilename() + ":" + "No such method:" + sme.getMessage(), sme);
        ...
        {
            if (DEBUG)
                Log.e(TAG, getLoggingFilename() + ":" + "Instantiation exception:" + ie.getMessage(), ie);
        ...
        {
            if (DEBUG)
                Log.e(TAG, getLoggingFilename() + ":" + "Illegal access exception :" + iae.getMessage(), iae);
            throw new RuntimeException(iae.getMessage());
        }
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":" + "Created framebody:end" + frameBody.getIdentifier());
        frameBody.setHeader(this);
        return frameBody;
    }
    ...
    protected String readIdentifier(ByteBuffer byteBuffer) throws PaddingException,InvalidFrameException
    {
        ...
        try {
            identifier = new String(buffer, "ISO-8859-1");
            if (DEBUG)
                Log.i(TAG, getLoggingFilename() + ":" + "Identifier is " + identifier + ", frame ID size = " + frameIdSize);
    ...
    protected AbstractID3v2FrameBody readBody(String identifier, AbstractID3v2FrameBody body) throws InvalidFrameException
    {
        ...
        {
            if (DEBUG)
                Log.i(TAG, "Identifier not recognised:" + identifier + " unable to create framebody");
        ...
        {
            if (DEBUG)
                Log.e(TAG, "No such method:" + sme.getMessage(), sme);
            throw new InvalidFrameException("FrameBody" + identifier + " does not have a constructor that takes:" + body.getClass().getName());
        }
        catch (InvocationTargetException ite)
        {
            if (DEBUG) {
                Log.e(TAG, "An error occurred within abstractID3v2FrameBody");
                Log.e(TAG, "Invocation target exception:" + ite.getCause().getMessage(), ite.getCause());
            }
            if (ite.getCause() instanceof Error)
            {
                throw (Error) ite.getCause();
        ...
        {
            if (DEBUG)
                Log.e(TAG, "Instantiation exception:" + ie.getMessage(), ie);
            throw new RuntimeException(ie.getMessage());
        }
        //Private Constructor shouild not happen
        catch (IllegalAccessException iae)
        {
            if (DEBUG)
                Log.e(TAG, "Illegal access exception :" + iae.getMessage(), iae);
            throw new RuntimeException(iae.getMessage());
        }

        if (DEBUG)
            Log.i(TAG, "frame Body created" + frameBody.getIdentifier());
        frameBody.setHeader(this);
        return frameBody;
    }
