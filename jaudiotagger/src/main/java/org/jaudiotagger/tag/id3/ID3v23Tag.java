...
{
    private static final String TAG = "ID3v23Tag";
    ...
    protected void addFrame(AbstractID3v2Frame frame)
    {
        ...
        {
            if (DEBUG)
                Log.e(TAG, "Unable to convert frame:" + frame.getIdentifier());
        }
    }
    ...
    protected void translateFrame(AbstractID3v2Frame frame)
    {
        ...
        if (!tmpBody.getYear().equals(""))
        {
            ...
            if (DEBUG)
                Log.i(TAG, "Adding Frame:" + newFrame.getIdentifier());
    ...
    public ID3v23Tag(ID3v23Tag copyObject)
    {
        ...
        super(copyObject);
        if (DEBUG)
            Log.i(TAG, "Creating tag from another tag of same type");
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Creating tag from a tag of a different version");
        frameMap = new LinkedHashMap();
        encryptedFrameMap = new LinkedHashMap();
        ...
        if (mp3tag != null)
        {
            ...
            if (DEBUG)
                Log.i(TAG, "Created tag from a tag of a different version");
        }
    }
    ...
    private void readHeaderFlags(ByteBuffer buffer) throws TagException
    {
        ...
        if (DEBUG) {
            ...
            if ((flags & FileConstants.BIT4) != 0) {
                Log.e(TAG, ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg(getLoggingFilename(), FileConstants.BIT4));
            }

            if ((flags & FileConstants.BIT3) != 0) {
                Log.e(TAG, ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg(getLoggingFilename(), FileConstants.BIT3));
            }

            if ((flags & FileConstants.BIT2) != 0) {
                Log.e(TAG, ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg(getLoggingFilename(), FileConstants.BIT2));
            }

            if ((flags & FileConstants.BIT1) != 0) {
                Log.e(TAG, ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg(getLoggingFilename(), FileConstants.BIT1));
            }

            if ((flags & FileConstants.BIT0) != 0) {
                Log.e(TAG, ErrorMessage.ID3_INVALID_OR_UNKNOWN_FLAG_SET.getMsg(getLoggingFilename(), FileConstants.BIT0));
            }

            if (isUnsynchronization()) {
                Log.i(TAG, ErrorMessage.ID3_TAG_UNSYNCHRONIZED.getMsg(getLoggingFilename()));
            }

            if (extended) {
                Log.i(TAG, ErrorMessage.ID3_TAG_EXTENDED.getMsg(getLoggingFilename()));
            }

            if (experimental) {
                Log.i(TAG, ErrorMessage.ID3_TAG_EXPERIMENTAL.getMsg(getLoggingFilename()));
            }
        }
    }
    ...
    private void readExtendedHeader(ByteBuffer buffer, int size)
    {
        ...
        if (extendedHeaderSize == TAG_EXT_HEADER_DATA_LENGTH)
        {
            ...
            {
                if (DEBUG)
                    Log.e(TAG, ErrorMessage.ID3_TAG_CRC_FLAG_SET_INCORRECTLY.getMsg(getLoggingFilename()));
            }
            ...
            {
                if (DEBUG)
                    Log.i(TAG, ErrorMessage.ID3_TAG_PADDING_SIZE.getMsg(getLoggingFilename(),paddingSize));
            }
        ...
        {
            if (DEBUG)
                Log.i(TAG, ErrorMessage.ID3_TAG_CRC.getMsg(getLoggingFilename()));
            ...
            crc32 = buffer.getInt();
            if (DEBUG)
                Log.i(TAG, ErrorMessage.ID3_TAG_CRC_SIZE.getMsg(getLoggingFilename(),crc32));
        }
        ...
        {
            if (DEBUG)
                Log.e(TAG, ErrorMessage.ID3_EXTENDED_HEADER_SIZE_INVALID.getMsg(getLoggingFilename(), extendedHeaderSize));
    ...
    public void read(ByteBuffer buffer) throws TagException
    {
        ...
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":" + "Reading ID3v23 tag");
        ...
        if (DEBUG)
            Log.i(TAG, ErrorMessage.ID_TAG_SIZE.getMsg(getLoggingFilename(),size));
        ...
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":Loaded Frames,there are:" + frameMap.keySet().size());
    ...
    protected void readFrames(ByteBuffer byteBuffer, int size)
    {
        ...
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":Start of frame body at:" + byteBuffer.position() + ",frames data size is:" + size);
        ...
        while (byteBuffer.position() < size)
        {
            ...
            try
            {
                //Read Frame
                if (DEBUG)
                    Log.i(TAG, getLoggingFilename() + ":Looking for next frame at:" + byteBuffer.position());
            ...
            {
                if (DEBUG)
                    Log.i(TAG, getLoggingFilename() + ":Found padding starting at:" + byteBuffer.position());
                break;
            }
            ...
            {
                if (DEBUG)
                    Log.e(TAG, getLoggingFilename() + ":Empty Frame:" + ex.getMessage());
                this.emptyFrameBytes += ID3v23Frame.FRAME_HEADER_SIZE;
            }
            catch (InvalidFrameIdentifierException ifie)
            {
                if (DEBUG)
                    Log.e(TAG, getLoggingFilename() + ":Invalid Frame Identifier:" + ifie.getMessage());
                this.invalidFrames++;
            ...
            {
                if (DEBUG)
                    Log.e(TAG, getLoggingFilename() + ":Invalid Frame:" + ife.getMessage());
                this.invalidFrames++;
                //Don't try and find any more frames
                break;
            }
            ...
            {
                if (DEBUG)
                    Log.e(TAG, getLoggingFilename() + ":Corrupt Frame:" + idete.getMessage());
                this.invalidFrames++;
                continue;
            }
        }
    ...
    public long write(File file, long audioStartLocation) throws IOException
    {
        ...
        if (DEBUG)
            Log.i(TAG, "Writing tag to file:"+getLoggingFilename());

        //Write Body Buffer
        byte[] bodyByteBuffer = writeFramesToBuffer().toByteArray();
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":bodybytebuffer:sizebeforeunsynchronisation:" + bodyByteBuffer.length);
        ...
        if (isUnsynchronization())
        {
            ...
            if (DEBUG)
                Log.i(TAG, getLoggingFilename() + ":bodybytebuffer:sizeafterunsynchronisation:" + bodyByteBuffer.length);
        }
        ...
        if (DEBUG) {
            Log.i(TAG, getLoggingFilename() + ":Current audiostart:" + audioStartLocation);
            Log.i(TAG, getLoggingFilename() + ":Size including padding:" + sizeIncPadding);
            Log.i(TAG, getLoggingFilename() + ":Padding:" + padding);
        }
    ...
    {
        if (DEBUG)
            Log.i(TAG, getLoggingFilename() + ":Writing tag to channel");
