...
{
    private static final String TAG = "TextEncodedStringSzTerm";
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Reading from array from offset:" + offset);
        ...
        {
            if (DEBUG)
                Log.e(TAG, "Decoding error:" + coderResult.toString());
        }
        ...
        {
            if (outBuffer.length() > 2 && outBuffer.get(0) == (char)0xFE && outBuffer.get(1) == (char)0xFF) {
                if (DEBUG)
                    Log.i(TAG, "Adjust BOM to Big Endian");

                outBuffer.put(0, (char)0xFF);
                outBuffer.put(1, (char)0xFE);
            }
        }
        ...
        if (DEBUG)
            Log.i(TAG, "Read SizeTerminatedString:" + value + " size:" + size);
    }
    ...
    public byte[] writeByteArray()
    {
        ...
        {
            if (DEBUG)
                Log.e(TAG, ce.getMessage()+":"+charSetName+":"+value);
    ...
    protected String getTextEncodingCharSet()
    {
        ...
        if (DEBUG)
            Log.i(TAG, "text encoding:" + textEncoding + " charset:" + charSetName);
        return charSetName;
    }
