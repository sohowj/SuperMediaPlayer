...
public class MP3File extends AudioFile
{
    ...
    private void readV1Tag(File file, RandomAccessFile newFile, int loadOptions) throws IOException
    {
        ...
        {
            if (DEBUG)
                Log.i(TAG, "Attempting to read id3v1tags");
            try
            {
            ...
            {
                if (DEBUG)
                    Log.e(TAG, "No ids3v11 tag found");
            }
    ...
    private void readV2Tag(File file, int loadOptions, int startByte) throws IOException, TagException
    {
        ...
        {
            if (DEBUG)
                Log.i(TAG, "Attempting to read id3v2tags");
        ...
        {
            if (DEBUG)
                Log.e(TAG, "Not enough room for valid id3v2 tag:" + startByte);
        }
    }
    ...
    {
        if (DEBUG)
            Log.i(TAG, "Checking file portion:" + Hex.asHex(startByte) + ":" + Hex.asHex(endByte));
        FileInputStream fis=null;
        FileChannel     fc=null;
        try
        {
    ...
    private MP3AudioHeader checkAudioStart(long startByte, MP3AudioHeader firstHeaderAfterTag) throws IOException, InvalidAudioFrameException
    {
        ...
        if (DEBUG)
            Log.i(TAG, ErrorMessage.MP3_ID3TAG_LENGTH_INCORRECT.getMsg(file.getPath(), Hex.asHex(startByte), Hex.asHex(firstHeaderAfterTag.getMp3StartByte())));
        ...
        Log.i(TAG, "Checking from start:" + headerOne);
        ...
        {
            if (DEBUG)
                Log.i(TAG, ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg(file.getPath(),
        ...
        else
        {
            ...
            if (DEBUG)
                Log.i(TAG, (ErrorMessage.MP3_RECALCULATED_POSSIBLE_START_OF_MP3_AUDIO.getMsg(file.getPath(),
                            Hex.asHex(headerOne.getMp3StartByte()))));
            ...
            {
                if (DEBUG)
                    Log.e(TAG, (ErrorMessage.MP3_RECALCULATED_START_OF_MP3_AUDIO.getMsg(file.getPath(),
                                Hex.asHex(headerOne.getMp3StartByte()))));
                return headerOne;
            }
            ...
            {
                if (DEBUG)
                    Log.e(TAG, (ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg(file.getPath(),
    ...
    public MP3File(File file, int loadOptions, boolean readOnly) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        ...
        try
        {
            ...
            if (DEBUG)
                Log.i(TAG, "TagHeaderSize:" + Hex.asHex(tagSizeReportedByHeader));
            ...
            {
                if (DEBUG)
                    Log.i(TAG, "First header found after tag:" + audioHeader);
    ...
    */
    ...
    {
        if (DEBUG)
            Log.i(TAG, "setting tagv1:v1 tag");
    ...
    {
        if (DEBUG)
            Log.i(TAG, "setting tagv1:abstract");
    ...
    */
    ...
    public void save(File fileToSave) throws IOException
    {
        ...
        try
        {
            ...
            if (TagOptionSingleton.getInstance().isId3v2Save())
            {
                if (id3v2tag == null)
                {
                    ...
                    if (DEBUG)
                        Log.i(TAG, "Deleting ID3v2 tag:"+file.getName());
                    rfile.close();
                }
                else
                {
                    if (DEBUG)
                        Log.i(TAG, "Writing ID3v2 tag:"+file.getName());
                    ...
                    if (mp3StartByte != newMp3StartByte) {
                        if (DEBUG)
                            Log.i(TAG, "New mp3 start byte: " + newMp3StartByte);
            ...
            {
                if (DEBUG)
                    Log.i(TAG, "Processing ID3v1");
                if (id3v1tag == null)
                {
                    if (DEBUG)
                        Log.i(TAG, "Deleting ID3v1");
                    (new ID3v1Tag()).delete(rfile);
                }
                else
                {
                    if (DEBUG)
                        Log.i(TAG, "Saving ID3v1");
        ...
        {
            if (DEBUG)
                Log.e(TAG, ErrorMessage.GENERAL_WRITE_FAILED_BECAUSE_FILE_NOT_FOUND.getMsg(file.getName()), ex);
            throw ex;
        }
        catch (IOException iex)
        {
            if (DEBUG)
                Log.e(TAG, ErrorMessage.GENERAL_WRITE_FAILED_BECAUSE.getMsg(file.getName(), iex.getMessage()), iex);
            throw iex;
        }
        catch (RuntimeException re)
        {
            if (DEBUG)
                Log.e(TAG, ErrorMessage.GENERAL_WRITE_FAILED_BECAUSE.getMsg(file.getName(), re.getMessage()), re);
            throw re;
        }
        finally
        {
